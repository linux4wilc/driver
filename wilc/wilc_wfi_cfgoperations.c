		       ...
		       struct cfg80211_chan_def *chandef)
{
	...
	if (result != 0)
		netdev_err(priv->dev, "Error in setting channel %d\n", channelnum);
		   ...
		   struct cfg80211_connect_params *sme)
{
	...
	if (s32Error != 0) {
		netdev_err(dev, "wilc_set_join_req(): Error(%d)\n", s32Error);
		s32Error = -ENOENT;
		wilc_connecting = 0;
		return s32Error;
	}
...
static int disconnect(struct wiphy *wiphy, struct net_device *dev, u16 reason_code)
{
	...
	if (s32Error != 0) {
		netdev_err(priv->dev, "Error in disconnecting: Error(%d)\n", s32Error);
		s32Error = -EINVAL;
	}
...

{
	...
	default:
		netdev_err(netdev, "Not supported cipher: Error(%d)\n", s32Error);
		s32Error = -ENOTSUPP;
	}
		       ...
		       const u8 *mac, struct station_info *sinfo)
{
	...
	if (vif->iftype == STATION_MODE) {
		...
		if (!wilc->initialized) {
			PRINT_D(CFG80211_DBG,"driver not initialized\n");
			return -EBUSY;
		}
...
static void WILC_WFI_CfgParseTxAction(u8 * buf,u32 len,bool bOperChan, u8 p2p_mode)
{
	u32 index = 0;
	u32 i = 0, j = 0;
							...
							if ((buff[P2P_PUB_ACTION_SUBTYPE] == GO_NEG_REQ || buff[P2P_PUB_ACTION_SUBTYPE] == GO_NEG_RSP ||
								...
								for (i = P2P_PUB_ACTION_SUBTYPE + 2; i < size; i++) {
									if (buff[i] == P2PELEM_ATTR_ID && !(memcmp(p2p_oui, &buff[i + 2], 4))) {
										WILC_WFI_CfgParseRxAction(&buff[i + 6], size - (i + 6), vif->attr_sysfs.p2p_mode);
										break;
									}
								}
							}
								...
								for (i = P2P_PUB_ACTION_SUBTYPE + 2; i < len; i++) {
									if (buf[i] == P2PELEM_ATTR_ID && !(memcmp(p2p_oui, &buf[i + 2], 4))) {
										if (buf[P2P_PUB_ACTION_SUBTYPE] == P2P_INV_REQ || buf[P2P_PUB_ACTION_SUBTYPE] == P2P_INV_RSP)
											WILC_WFI_CfgParseTxAction(&mgmt_tx->buff[i + 6], len - (i + 6), true, vif->attr_sysfs.p2p_mode);
										else
											WILC_WFI_CfgParseTxAction(&mgmt_tx->buff[i + 6], len - (i + 6), false, vif->attr_sysfs.p2p_mode);
										break;
									}
								}
